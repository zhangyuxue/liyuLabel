...
AbstractItemResizeHandles *ResizeHandlesFactory::createResizeHandles(AbstractAnnotationItem *item)
{
    ...
    resizeHandles = GetPolygonResizeHandles(item);
    if (resizeHandles != nullptr) {
        return resizeHandles;
    }

    resizeHandles = GetNumberGroupResizeHandles(item);
    if (resizeHandles != nullptr) {
        return resizeHandles;
    }
...
AbstractItemResizeHandles *ResizeHandlesFactory::GetPolygonResizeHandles(AbstractAnnotationItem *item)
{
    auto pathItem = dynamic_cast<AbstractAnnotationPolygon *>(item);
    if (pathItem != nullptr) {
        return new PolygonResizeHandles(pathItem);
    }
    return nullptr;
}

AbstractItemResizeHandles *ResizeHandlesFactory::GetNumberGroupResizeHandles(AbstractAnnotationItem *item)
{
    auto pathItem = dynamic_cast<AbstractAnnotationNumberGroup *>(item);
    if (pathItem != nullptr) {
        return new NumberGroupResizeHandles(pathItem);
    }
    return nullptr;
}
